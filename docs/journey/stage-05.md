# Stage 05 · Name and type binding
...
??? note "File diff: tests/unit/sql/test_binder_aggregates.py"
    ```diff
...
??? note "File diff: tests/unit/sql/test_binder_types.py"
    ```diff
...
??? note "File diff: src/minipostgres/sql/bound.py"
    ```diff
