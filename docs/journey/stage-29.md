# Stage 29 · Cross-layer correctness regressions
...
??? note "File diff: tests/unit/executor/test_expressions.py"
    ```diff
...
??? note "File diff: tests/unit/executor/test_query_operators.py"
    ```diff
...
??? note "File diff: tests/unit/sql/test_binder_names.py"
    ```diff
...
??? note "File diff: src/minipostgres/executor/expressions.py"
    ```diff
