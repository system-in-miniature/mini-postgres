# Stage 01 · Value and row contract
...
??? note "File diff: tests/unit/test_rows.py"
    ```diff
...
??? note "File diff: tests/unit/test_types.py"
    ```diff
...
??? note "File diff: src/minipostgres/row.py"
    ```diff
...
??? note "File diff: src/minipostgres/types.py"
    ```diff
