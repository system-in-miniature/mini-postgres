# Stage 09 · Validated DML query loop
...
??? note "File diff: tests/contract/test_constraints.py"
    ```diff
...
??? note "File diff: tests/contract/test_database_api.py"
    ```diff
...
??? note "File diff: tests/integration/test_join_aggregate.py"
    ```diff
...
??? note "File diff: tests/unit/executor/test_modify_operators.py"
    ```diff
