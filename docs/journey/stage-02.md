# Stage 02 · Durable typed catalog
...
??? note "File diff: tests/integration/test_catalog_restart.py"
    ```diff
...
??? note "File diff: tests/unit/catalog/test_model.py"
    ```diff
...
??? note "File diff: src/minipostgres/catalog/model.py"
    ```diff
